...
// Notebooks
export const createNotebookApi = async (
    req: NotebookRequest
): Promise<NotebookResponse> => {
    const res = await client.post<ApiResponse<NotebookResponse>>(
        "/notebooks",
        req
    )
...
export const createChapterApi = async (
    ...
    const res = await client.post<ApiResponse<ChapterResponse>>(
        "/chapters",
        req
    )
    return res.data.data!
...
export const getChaptersApi = async (
    ...
    if (!notebookIds)
        throw new Error("getChaptersApi: No notebook ids provided.")
    const res = await client.post<ApiResponse<ChapterResponse[]>>(
        "/chapters/by-notebooks",
        {
            notebookIds,
        }
    )
...
// Canvases
export const createCanvasApi = async (
    req: CanvasRequest
...
export const getCanvasesApi = async (
    ...
    const res = await client.post<ApiResponse<CanvasResponse[]>>(
        "/canvases/by-chapters",
        {
            chapterIds,
        }
    )
...
export const getPathsApi = async (chapterId: String) => {
	const res = await client.get<ApiResponse<PathResponse[]>> ("/paths/by-chapter", {
		params: {chapterId}
	})
	console.log("Paths:", JSON.stringify(res.data, null, 2))
	return res.data.data!
